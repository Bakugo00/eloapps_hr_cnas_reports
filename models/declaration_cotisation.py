...
class DeclarationCotisation(models.Model):
    _name = "declaration.cotisation"
    ...
    line_cotisation_ids = fields.One2many('cotisation.line', 'line_cotisation_id', string='Lignes de cotisation')
    ...
    abatement40_id = fields.One2many('abatement.base','abatement40_id', string='Abattement 40%')
    abatement80_id = fields.One2many('abatement.base','abatement80_id', string='Abattement 80%')
    abatement90_id = fields.One2many('abatement.base','abatement90_id', string='Abattement 90%')

    employee_movement_id = fields.One2many('movement.employees','employee_movement_id', string='Mouvement des salariés')
    ...
    lot_declaration_cotisation_id = fields.Many2one('lot.declaration.cotisation', string='Lot de déclaration des cotisations')
    ...
    def calculer_lignes_cotisation(self):
        ...
        sublines = self.env['hr.payslip.line'].search([
        ...
        if len(emplye_06) != 0:
            for emp_06 in emplye_06:
                some_cot_emp = 0
                emp_lines = self.env['hr.payslip.line'].search([
    ...
    def print_repport(self):
        return self.env.ref('eloapps_hr_cnas_reports.declaration_cotisation_repport').report_action(self)

    def unlink(self):
        for rec in self:
            if rec.state == "confirme":
                raise ValidationError("Vous ne pouvez pas supprimer une déclaration confirmé.")
        res = super(DeclarationCotisation, self).unlink()
        return res

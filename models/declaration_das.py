...
class DeclarationDas(models.Model):
    _name = "declaration.das"
    ...
    declaration_das_line_ids = fields.One2many('declaration.das.line','declaration_das_id', 'Ligne de déclaration annuelle des salariés',relation="das_lines1")
    declaration_das_line2_ids = fields.One2many('declaration.das.line','declaration_das_id', 'Ligne de déclaration annuelle des salariés',relation="das_lines2")
    declaration_das_line3_ids = fields.One2many('declaration.das.line','declaration_das_id', 'Ligne de déclaration annuelle des salariés',relation="das_lines3")
    ...
    def create(self, vals):
        if vals.get('name', 'New') == 'New':
            vals['name'] = "Déclaration annuelle des salariés "+ vals.get('periode')

        return super(DeclarationDas ,self).create(vals)
    ...
    def generate_company_file(self):
        ...
        for ac in self.company_id.partner_id.activity_code:
    ...
    def generate_employee_file(self):
        ...
        action = self.env.ref('eloapps_hr_cnas_reports.action_hr_employee_list').read()[0]
    ...
    def print_report(self):
        return self.env.ref('eloapps_hr_cnas_reports.declaration_das_repport').report_action(self)

    def calcule_lines(self):

        line = self.env['hr.payslip.line']
        worked = self.env['hr.payslip.worked_days']
        ...
        for payslip in payslips:
            ...
            date_start = self.env['hr.contract'].search([('employee_id', '=', payslip.employee_id.id)],order="date_start asc" ,limit=1).date_start
            date_end = self.env['hr.contract'].search([('employee_id', '=', payslip.employee_id.id)],order="date_start desc" ,limit=1).date_end
            if payslip.employee_id not in values.keys():
                values.update({payslip.employee_id :
                {
                    'name': payslip.employee_id.sec_social,
        ...
        for val in values :
            ...
            self.declaration_das_line_ids += self.env['declaration.das.line'].create(values[val])
    ...
    def generate_rtf(self):
        ...
        for i in range(len(total)):
            table_lines.append([
                total[i].employee_id.sec_social if total[i].employee_id.sec_social else "____",
...
class DeclarationDasLines(models.Model):
    _name = "declaration.das.line"
